...
local script_Version = "0.1"               -- Версия скрипта
---
...
local mainIni = inicfg.load({ -- CFG
    mode = 1, 
    settings = {
        theme = 1, -- По умолчанию 1 (фиолетовая тема)
        ukrop_map = false, -- Состояние карты наркотиков
        online_check = false, -- Переключатель чекера банды
        online_check_x = 76, -- Координата X по умолчанию
        online_check_y = 450, -- Координата Y по умолчанию
        fps_ping_display = false, -- Состояние отображения FPS и PING
        fps_ping_x = 107, -- Позиция по X (по умолчанию)
        fps_ping_y = 1044, -- Позиция по Y (по умолчанию)
        rp_invite = false,
        prospammer = false,
        spawn_cars = true,
        fill_cars = true, 
...
local theme_is_dark = imgui.new.bool(mainIni.settings.theme == 2)
local online_check = imgui.new.bool(mainIni.settings.online_check)
local online_check_x = mainIni.settings.online_check_x or 76
local online_check_y = mainIni.settings.online_check_y or 450
local changePositionMode = false -- Режим изменения позиции
local fps_ping_display = imgui.new.bool(mainIni.settings.fps_ping_display)
local fpsPingX = mainIni.settings.fps_ping_x or 107
local fpsPingY = mainIni.settings.fps_ping_y or 1044
local fpsPingChangePosition = false -- Режим изменения позиции
...
function main()
  while not isSampAvailable() do wait(100) end
    ...
    autoupdate("https://raw.githubusercontent.com/Drey4ik/GHelper/refs/heads/main/AutoUpdate.GHelper", tag.. "")
  ...
  while true do 
    ...
    if changePositionMode then
        sampToggleCursor(true) -- Включаем курсор
        local cursorX, cursorY = getCursorPos()
        online_check_x = cursorX
        online_check_y = cursorY

        if isKeyJustPressed(VK_LBUTTON) then
            changePositionMode = false
            sampToggleCursor(false) -- Выключаем курсор
            saveSettings()
        end
    end
    if fpsPingChangePosition then
        sampToggleCursor(true) -- Включаем курсор
        local cursorX, cursorY = getCursorPos()
        fpsPingX = cursorX
        fpsPingY = cursorY
        ...
        if isKeyJustPressed(VK_LBUTTON) then
            fpsPingChangePosition = false
            sampToggleCursor(false) -- Выключаем курсор
            saveSettings() -- Сохраняем новую позицию
        end
    end
    if rp_invite[0] then
        ...
        if result and isKeyDown(0x45) then 
            ...
            wait(1000)
            sampAddChatMessage(tag..'Приглашение отправлено игроку: {9461FF}'..name..'{ffffff}. Ожидайте ответа, затем сможете пригласить следующего.', -1)
            active = true
...
imgui.OnFrame(function()return window[0]end, function(player)
    if window[0] then
        ...
        elseif activeButton == 2 then  -- Функции
            ...
            imgui.Spacing()
            if imgui.ToggleButton(u8'Мониторинг', fps_ping_display) then end
            imgui.SameLine()
            imgui.TextQuestion(u8'Показывает текущий FPS и PING в игре.') 
            if fps_ping_display[0] then
                if imgui.Button(faicons('arrows_maximize') .. u8(' Изменить позицию '), imgui.ImVec2(140, 20)) then
                    fpsPingChangePosition = true
                    sampAddChatMessage(tag .. "Перетащите текст в нужную область и щелкните LMB для сохранения.", -1)
                end
            end
            imgui.Spacing()
            if imgui.ToggleButton(u8'Состав банд', online_check) then end
            imgui.SameLine()
            imgui.TextQuestion(u8'Показывает текущий онлайн в бандах.')   
            if online_check[0] then
                if imgui.Button(faicons('arrows_maximize') .. u8(' Изменить позицию'), imgui.ImVec2(140, 20)) then
        ...
        elseif activeButton == 4 then  -- Настройки
            if imgui.ToggleButton(u8("Тёмная тема"), theme_is_dark) then
                mainIni.settings.theme = theme_is_dark[0] and 2 or 1
                Theme(mainIni.settings.theme)
            end
            imgui.SameLine()  
            imgui.TextQuestion(u8'Смена стиля интерфейса на тёмную тему.')
            imgui.Dummy(imgui.ImVec2(0, 195))
            if imgui.Button(faicons('circle_dollar_to_slot') .. u8(' Поддержать автора'), imgui.ImVec2(270, 20)) then
                sampAddChatMessage(tag..'Извините, функция ещё в разработке.', -1)
            end
            imgui.SameLine() 
            if imgui.Button(faicons('power_off') .. u8(' Выключить GHelper'), imgui.ImVec2(270, 20)) then
                showCursor(false) thisScript():unload()
            end
        ...
        imgui.SameLine()
        if imgui.Button(faicons('circle_xmark') .. u8(' Закрыть'), imgui.ImVec2(228, 39)) then
            window[0] = false
        end
...
function saveSettings()
    mainIni.settings.theme = theme_is_dark[0] and 2 or 1
    mainIni.settings.online_check = online_check[0]
    mainIni.settings.online_check_x = online_check_x
    mainIni.settings.online_check_y = online_check_y
    ...
    mainIni.settings.fps_ping_display = fps_ping_display[0]
    mainIni.settings.fps_ping_x = fpsPingX
    mainIni.settings.fps_ping_y = fpsPingY
...
local font
local factionColors = {
    {color = 0x99009327, name = "Grove", hex = "009327"},
    {color = 0x99CC00CC, name = "Ballas", hex = "CC00CC"},
    {color = 0x996666FF, name = "Rifa", hex = "6666FF"},
    {color = 0x99D1DB1C, name = "Vagos", hex = "D1DB1C"},
    {color = 0x9900FFE2, name = "Aztecaz", hex = "00FFE2"},
    {color = 0x80A87878, name = "Wolfs", hex = "7F6464"}
}

function OnlineCheckBand()
    if not font then
        font = renderCreateFont("Arial", 11, 5)
    end
    local factionCounts = {}
    for _, faction in ipairs(factionColors) do
        factionCounts[faction.name] = 0
    end
    for l = 0, 1004 do
        if sampIsPlayerConnected(l) then
            local playerColor = sampGetPlayerColor(l)
            
            -- Используем таблицу для быстрого поиска фракции
            for _, faction in ipairs(factionColors) do
                if playerColor == faction.color then
                    factionCounts[faction.name] = factionCounts[faction.name] + 1
                    break
                end
            end
        end
    ...
    local text = ""
    for _, faction in ipairs(factionColors) do
        -- Формируем текст с цветом фракции и количеством игроков
        text = text .. string.format("{%s}%s: {ffffff}%d\n", 
            faction.hex, 
            faction.name, 
            factionCounts[faction.name])
    end
    renderFontDrawText(font, text, online_check_x, online_check_y, -1)
end
---------------- показывает фпс и пинг
local fpsFont
local lastTime = 0
local framesInCurrentSecond = 0
local currentFPS = 0
local playerID = nil -- Местоположение для ID игрока

function FPSandPING()
    if not fpsFont then
        fpsFont = renderCreateFont("Arial", 11, 5)
    end
    ...
    local pingText = ping == 0 and "Ping: Н/Д" or string.format("PING: %d", ping)
    local text = string.format("FPS: %d    %s", currentFPS, pingText)
    local textColor = 0xFFFFFFFF -- Белый
    local shadowColor = 0xFF000000 -- Черная тень
    renderFontDrawText(fpsFont, text, fpsPingX + 1, fpsPingY + 1, shadowColor) -- Тень
    renderFontDrawText(fpsFont, text, fpsPingX, fpsPingY, textColor) -- Основной текст
end
...
function Theme(theme)
    ...
    elseif theme == 2 then
        ...
        colors[clr.PopupBg]                  = ImVec4(0.06, 0.06, 0.06, 0.94);
        colors[clr.Border]                   = ImVec4(0.13, 0.13, 0.13, 1.00);
        colors[clr.BorderShadow]             = ImVec4(1.00, 1.00, 1.00, 0.10);
        colors[clr.FrameBg]                  = ImVec4(0.20, 0.20, 0.20, 1.00);
        colors[clr.FrameBgHovered]           = ImVec4(0.18, 0.18, 0.18, 1.00);
        colors[clr.FrameBgActive]            = ImVec4(0.15, 0.15, 0.15, 1.00);
        colors[clr.TitleBg]                  = ImVec4(0.10, 0.10, 0.10, 0.81);
        colors[clr.TitleBgActive]            = ImVec4(0.13, 0.13, 0.13, 1.00);
        colors[clr.TitleBgCollapsed]         = ImVec4(0.05, 0.05, 0.05, 0.51);
        colors[clr.MenuBarBg]                = ImVec4(0.14, 0.14, 0.14, 1.00);
        colors[clr.ScrollbarBg]              = ImVec4(0.02, 0.02, 0.02, 0.39);
        colors[clr.ScrollbarGrab]            = ImVec4(0.33, 0.33, 0.33, 1.00);
        colors[clr.ScrollbarGrabHovered]     = ImVec4(0.25, 0.25, 0.25, 1.00);
        colors[clr.ScrollbarGrabActive]      = ImVec4(0.20, 0.20, 0.20, 1.00);
        colors[clr.CheckMark]                = ImVec4(0.55, 0.55, 0.55, 1.00);
        colors[clr.SliderGrab]               = ImVec4(0.30, 0.30, 0.30, 1.00);
        colors[clr.SliderGrabActive]         = ImVec4(0.25, 0.25, 0.25, 1.00);
        colors[clr.Button]                   = ImVec4(0.20, 0.20, 0.20, 1.00);
        colors[clr.ButtonHovered]            = ImVec4(0.18, 0.18, 0.18, 1.00);
        colors[clr.ButtonActive]             = ImVec4(0.15, 0.15, 0.15, 1.00);
        colors[clr.Header]                   = ImVec4(0.20, 0.20, 0.20, 1.00);
        colors[clr.HeaderHovered]            = ImVec4(0.22, 0.22, 0.22, 1.00);
        colors[clr.HeaderActive]             = ImVec4(0.18, 0.18, 0.18, 1.00);
        colors[clr.Tab]                      = ImVec4(0.18, 0.18, 0.18, 1.00);
        colors[clr.TabHovered]               = ImVec4(0.22, 0.22, 0.22, 1.00);
        colors[clr.TabActive]                = ImVec4(0.15, 0.15, 0.15, 1.00);
        colors[clr.TextSelectedBg]           = ImVec4(0.35, 0.35, 0.35, 1.00);
    end
end
